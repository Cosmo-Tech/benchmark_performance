...
from utils.constants import PERFORMANCE_CAPACITY_CSV, PERFORMANCE_STEPS_CSV, PERFORMANCE_TEST_CSV
...
def export_results():
    offset_end=50
    interval=20
    filename_results = f'{PATH.LOGS}/{PERFORMANCE_TEST_CSV}'
    ...
    original_df_steps = pd.read_csv(f'{PATH.LOGS}/{PERFORMANCE_STEPS_CSV}')
    ...
    fig.suptitle(f"Result run test {PATH.zip_file}", fontsize=SUBTITLE_SIZE)
...
def export_results_detailed():
    ...
    dataframe_original = pd.read_csv(f'{PATH.LOGS}/{PERFORMANCE_TEST_CSV}')
    dataframe_original['start_date'] = pd.to_datetime(dataframe_original['start_date'])
    scenario_list = dataframe_original.groupby(['scenario_id'])
    ...
    if len(data) > 1:
        ...
        fig.suptitle(f"Results run test {PATH.zip_file}", fontsize=SUBTITLE_SIZE)
        ...
        fig.suptitle(f"Detailed Execution time by step {PATH.zip_file}", fontsize=SUBTITLE_SIZE)
...
def export_results_global():
    ...
    fig.suptitle(f"Total Execution time, {PATH.zip_file}", fontsize=SUBTITLE_SIZE)
    a_x.set_xlabel('Execution time (s)')
    a_x.set_xlim(left=0, right=x_max)
    a_x.grid('on', which='major', axis='x' )
    a_x.grid('on', which='major', axis='y' )
...
def export_main_report():
    ...
    fig.suptitle(f"Result performance test {PATH.zip_file}", fontsize=SUBTITLE_SIZE)
    #################################################
    for item in data:
        max_list = []
        size_list = []
    ...
    summary_headers = {
        ...
        "Scenario summary": f": {PATH.zip_file}",
        "Environement": {
    ...
    d_t.to_csv(f"{PATH.LOGS}/{PERFORMANCE_CAPACITY_CSV}", mode='a', header=True, index=False)
    axs[2].set_axis_off()
