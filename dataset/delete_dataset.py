...
from utils.constants import DATASET_FOLDER
...
async def delete_dataset_workspace(dataset_input: str, dataset_id: str):
    ...
    container_client = SERVICES.blob_service_client.get_container_client(organization_id_lower)
    directory_name_in_storage = f"{workspace_id_lower}/datasets/{dataset_id}"

    local_dir_dataset = f"{PATH.DATA}/{DATASET_FOLDER}/{dataset_input}"
